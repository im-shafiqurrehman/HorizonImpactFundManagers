...
const Verification: FC<Props> = ({ setRoute }) => {
  ...
  return (
    <div className="w-[65%] sm:w-[75%] max-w-[320px] mx-auto px-2 py-4 rounded-xl border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.07)]">
      ...
      <p className="text-center text-[#545454] text-[10px] mt-3 mb-4">
      ...
      <div className="mt-3 flex items-center justify-center gap-1">
        {Object.keys(verifyNumber).map((key, index) => (
          <input
            ...
            key={key}
            ref={inputRefs[index]}
            className={`w-6 h-8 bg-transparent border-[1.5px] rounded-md text-center text-sm font-bold outline-none text-[#545454] ${
              invalidError ? "border-red-500 animate-shake" : "border-[#e9844c]/30 focus:border-[#e9844c]"
      ...
      <div className="w-full flex justify-center mt-5">
        <button className="w-full bg-[#e9844c] text-white font-medium py-1.5 rounded-md" onClick={verifyHandler}>
          Verify OTP
        </button>
      </div>

      <p className="text-center text-[10px] text-[#545454] mt-3">
        Go back to Sign in?{" "}
